...
export const NewDetailHistory = () => {
  ...
  return (
    <div className='bg-slate-100'>
        <ScrollToTop />
        <Navbar />
        {history.map.bookingId === bookingId ?
        <div key={history.bookingId} className='max-w-[1240px] md:px-14 mx-auto bg-slate-100 min-h-screen '>
        <div className='grid md:grid-cols md:grid-cols-[60%_40%] gap-2'>
          <div className='bg-white mt-20 rounded-md shadow-md'>
            <div className='bg-slate-100 py-2 px-2 mx-3 my-3 rounded-full'>
              <div className='flex justify-between'>
                <div className='flex font-bold items-center gap-1 mx-3'>
                  {history && history?.departure?.data.map(city => (
                    <>
                    ...
                    <div className='bg-blue-600 rounded-full text-white text-sm py-1 px-2'>{history?.bookingType}</div>
                </div>
                <div className='flex items-center'>
                  <div className='hidden md:flex text-gray-400 mx-3 font-medium'>Booking Id : {history?.bookingId}</div>
                </div>
              </div>
            </div>
            ...
            <div className='mx-8'>
              ...
              {history && history?.departure?.data.map((item,i) => (
                <div key={i}>
          ...
          {history && history?.departure?.data.slice(0,1).map((item,i) => (
          <div key={i} className='bg-white rounded-md shadow-md md:mt-20 px-5 py-5 '>
            ...
            <div className='bg-slate-100 rounded-lg px-2 py-2'>
              ...
              <div className='flex justify-between mt-5 mb-2 text-blue-600'>
                ...
                <div className='font-bold ite'>{history?.totalAmount?.display}</div>
              </div>
            </div>
            <p className='text-xs text-gray-400'>Ordered At {history?.orderedAt}</p>
