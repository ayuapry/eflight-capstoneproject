      ...
      {aircraft?.map((e, i) => (
        <div className="px-5 py-5">
          <div className="bg-white shadow-md px-5">
            <div className="py-3">
              <p>
                {e?.airlines?.airlineName} with id :{" "}
                <span className="text-gray-400">{e.id}</span>
              </p>
              <div className="gap-0">
                <p>
                  Model <span className="text-gray-400">{e.model}</span>{" "}
                  Manufacture:{" "}
                  <span className="text-gray-400">{e?.manufacture.name}</span>
                </p>
                <span>
                  Total unit : {e.totalUnit}, with passenger capacity{" "}
                  {e?.passengerCapacity} and Seat Arrangement{" "}
                  {e?.seatArrangement}{" "}
                </span>
              </div>
            </div>
